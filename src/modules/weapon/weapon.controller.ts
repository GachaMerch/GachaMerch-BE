...
import fs from "fs";
import { getWeapons, createWeapon, updateWeapon, deleteWeapon } from "./weapon.service";
import { successResponse, errorResponse } from "@/utils/response";

export const getWeaponsHandler = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const createWeaponHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    const { title, type, description, stock, rarity, baseAtk, price, discount, subStat, passiveName, passiveDesc } = req.body;
...
export const updateWeaponHandler = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const deleteWeaponHandler = async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id as string);
    if (isNaN(id)) {
      errorResponse(res, "Invalid weapon ID", 400);
